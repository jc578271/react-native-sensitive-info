...
RCT_EXPORT_METHOD(getAllItems:(NSDictionary *)options resolver:(RCTPromiseResolveBlock)resolve rejecter:(RCTPromiseRejectBlock)reject){
    ...
    if (accessGroup) {
        [query setObject:accessGroup forKey:(NSString *)kSecAttrAccessGroup];
    }
        ...
        if(result!=NULL){
            ...
            for (NSDictionary* item in (__bridge id)result) {
                ...
                @try
                {
                  ...
                  [finalItem setObject:(NSString*)[item objectForKey:(__bridge id)(kSecAttrAccessGroup)] forKey:@"accessGroup"];
